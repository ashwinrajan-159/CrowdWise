...
def main() -> None:
    ...
    # Stack history then new events; remember which rows are 'new'.
    ...
    print(f"[features] {feats.shape[0]} rows x {feats.shape[1]} columns "
          f"({int(is_new.sum())} upcoming)")

    # --- train on history, predict on the new events --------------------------
    # synthetic target = demonstrable delay signal for any row (history has no
    # native speed; this is the documented Phase-0 stand-in).
    tf = get_target_provider("synthetic").build(combined, feats, cfg)
    history_mask = ~is_new
    model = train_gbm(combined, feats, tf, cfg, train_mask=history_mask)
    pred = model.predict(
        combined, feats, tf.y, tf.observed,
        query_mask=is_new, history_mask=history_mask,
    )

    # --- generate the playbook for the upcoming events ------------------------
    ...
    # --- build the dashboard JSON + write the HTML ----------------------------
    view = _build_view(combined, feats, pred, pb, new_rep)
    out_path = Path(args.out)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    _render_html(Path(args.template), out_path, view)
